...
import { RepositoryFormValues } from '../components/HelmRepositories/types';
    ...
    async (url: string, ca: string): Promise<void> => {
      ...
      try {
        ...
        return Promise.reject(
          `Failed to update certificate authority: Failed to get hostname from url ${url}`,
        );
      }
      const newConfigMap: ConfigMap = { ...configMap, data: { ...configMap.data, [hostname]: ca } };
